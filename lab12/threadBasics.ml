...
let rec snooze s () = 
  ...
  Lwt_io.printl "WAKE UP YOU NEED TO MAKE MONEY" >>= fun () ->
  (snooze s ()) >>= fun () -> Lwt.return ()

(* And your definition of pick_out*)
let rec pick_out l = Lwt_list.iter_s (fun (a,b)-> if b=true then Lwt_io.printl (string_of_int a) else Lwt.return()) l
