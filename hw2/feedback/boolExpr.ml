...
let rec tokens lst = match lst with
  ...
  |h::t ->
      ...
      | "T" -> T::(tokens t)
      | "F" -> F::(tokens t)
      | s -> if (lowervars s) then (STR s)::(tokens t) else (tokens t)
...
let parse_bool_exp tl =
 let rec parser tl stk = match tl with
   ...
   | OPEN::NOT::more -> (match (parser more []) with e1::stk'-> stk @ [NotExpr(e1)] @ stk'| _ -> invalid_arg "Invalid Syntax1")
   | OPEN::AND::more -> (match (parser more []) with e1::e2::stk'-> stk@[AndExpr(e1,e2)] @ stk'| _ -> invalid_arg "Invalid Syntax2")
   | OPEN::OR::more -> (match (parser more []) with e1::e2::stk'-> stk@[OrExpr(e1,e2)] @ stk'| _ -> invalid_arg "Invalid Syntax3")
...
(* TA COMMENT(zhan4136) Lexing Feedback: 10/12 
lowervars returns true on "6" *)
